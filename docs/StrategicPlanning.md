...
## 5. How HermesAgency operationalizes this

### 5.1 `Goals.md` and `Guardrails.md` as three-layer documents
...
### 5.3 Skills and scripts as the input layer
...
### 5.4 Weekly testability cadence
...
## 6. A worked example (small business)
...
## 8. Where this lives in HermesAgency
...
## 9. The single-paragraph summary
...
between Initiative and Interim Goal. The plan is always part of
HermesAgency's operating background context, so every skill firing
happens with awareness of where it sits in the layered structure.
The audit surfaces drift at any layer. The weekly review answers
two questions: *are the inputs moving the outputs, and are the
outputs moving the outcomes?*

That discipline — structural, testable, never absent from the
operating context — is what makes HermesAgency a strategic
operations layer rather than just another AI assistant.
