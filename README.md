...
1. **Supervised learning loop** — captures every correction you give, propagates it to every relevant skill across the agency, and tells you when the loop breaks (so you stop repeating yourself). Your declared goals (in `Goals.md` and the other operational context docs) are always part of the background context the agency operates in. Your declared values (in `Guardrails.md`) are loaded into the enforcement layer — Sentinel, the audit, and the send-guard — so the agency is **aimed by Goals and checked by Guardrails**, not constrained by Guardrails on every turn
